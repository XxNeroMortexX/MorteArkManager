...
mkdir "H:\ark-manager\assets\css" 2>nul
mkdir "H:\ark-manager\assets\js" 2>nul
mkdir "H:\ark-manager\character-transfer" 2>nul
mkdir "H:\ark-manager\file-browser" 2>nul
mkdir "H:\ark-manager\includes" 2>nul
mkdir "H:\ark-manager\ini_comparison" 2>nul
mkdir "H:\ark-manager\ini-editor" 2>nul
mkdir "H:\ark-manager\logs" 2>nul
mkdir "H:\ark-manager\monitor" 2>nul
mkdir "H:\ark-manager\rcon" 2>nul
mkdir "H:\ark-manager\scripts" 2>nul
mkdir "H:\ark-manager\secure" 2>nul
mkdir "H:\ark-manager\server-control" 2>nul
mkdir "H:\ark-manager\settings" 2>nul
echo Done!

echo.
echo [2/7] Setting permissions on logs directory...
icacls "H:\ark-manager\logs" /grant "NT AUTHORITY\SYSTEM:(OI)(CI)F" /T >nul 2>&1
echo Done!
...
if exist "H:\apache24\bin\htpasswd.exe" (
    "H:\apache24\bin\htpasswd.exe" -c "H:\ark-manager\.htpasswd" %USERNAME%
    echo .htpasswd created successfully!
) else (
    echo WARNING: htpasswd.exe not found. You'll need to create .htpasswd manually.
    echo Location: H:\apache24\bin\htpasswd.exe
)

echo.
echo [6/7] Setting ARK directory permissions...
icacls "H:\ARKServers" /grant "NT AUTHORITY\SYSTEM:(OI)(CI)F" /T >nul 2>&1
echo Done!
...
echo NEXT STEPS:
echo ============================================
echo 1. Copy all PHP files to H:\ark-manager\
echo 2. Edit config.php with your server settings
echo 3. Move your batch files to H:\ARKServers\
